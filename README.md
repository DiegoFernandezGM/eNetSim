...
With the configuration loaded, the program initializes the Network class. The Network class is designed to represent the entire network of subsystems, which are dynamically created based on the configuration. The class uses a SubsystemRegistry to manage and instantiate these subsystems, ensuring that the network can be easily extended or modified by simply updating the configuration file.
