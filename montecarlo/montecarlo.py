        ...
        for param in self.input_params:
            if "flow_rate" in param:
                ...
                if isinstance(data, list) or isinstance(data, tuple):
                    if len(data) == 1:
                        param_distributions[param] = np.random.uniform(low=data*0.9, high=data*1.1, scale=10, size=1000)
                    else:
                        handler = DistributionAnalyzer(data)
                        param_distributions[param] = handler.analyze_and_sample()
                else:
                    param_distributions[param] = np.random.uniform(low=data*0.9, high=data*1.1, size=1000)
                ...
                if isinstance(data, list) or isinstance(data, tuple):
                    if len(data) == 1:
                        param_distributions[param] = np.random.uniform(low=data - 100, high=data + 100, size=1000)
                    else:
                        handler = DistributionAnalyzer(data)
                        param_distributions[param] = handler.analyze_and_sample()
                else:
                ...
                if isinstance(data, list) or isinstance(data, tuple):
                    if len(data) == 1:
                        param_distributions[param] = np.random.uniform(low=data*0.9, high=0.9, size=1000)
                    else:
                        handler = DistributionAnalyzer(data)
                        param_distributions[param] = handler.analyze_and_sample()
                else:
        ...
        if results:
            ...
            for subsystem_name in subsystem_emissions_mean.keys():
                ...
                if subsystem_emissions:
                    ...
                    #Violin plot
                    axs[2].violinplot([subsystem_emissions], showmeans=True, showmedians=True)
                    axs[2].set_title(f'Violin Plot - {subsystem_name}')
                    axs[2].set_xlabel('Probability Density')
                    axs[2].set_ylabel('Emissions (kg CO2)')

                    #CDF
                    axs[3].plot(np.sort(subsystem_emissions), np.linspace(0, 1, len(subsystem_emissions), endpoint=False))
                    axs[3].set_title(f'CPD Plot - {subsystem_name}')
                    axs[3].set_xlabel('Emissions (kg CO2)')
